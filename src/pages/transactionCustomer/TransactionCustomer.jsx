...
export default function TransactionCustomer() {
  ...
  const getTrans = async (phoneNumber) => {
    try {
      const res = await axios.get(
        `https://backend-apidoc.herokuapp.com/api/customer/getCustomerTrans/${phoneNumber}`,
        {
          headers: {
            authorization:
  ...
  const columns = [
    ...
    { field: "Status", headerName: "Status", width: 250 },
    {
      ...
      renderCell: (params) => {
        return (
          <>
            <Link to={`getTransactionDetail/${params.row.id}`}>
